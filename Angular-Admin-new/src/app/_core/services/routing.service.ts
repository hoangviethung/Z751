...
export class RoutingService {
	...
	public events: BehaviorSubject<RouterEvent> = new BehaviorSubject(
		undefined
	);
				...
				while (route.children.length) {
					...
					if (route.params || route.data) {
						for (const key in route.params) {
							...
							if (route.data['title']) {
								route.data['title'] = route.data[
									'title'
								].replace(`:${key}`, route.params[key]);
								route.data['title'] = route.data[
									'title'
								].replace(`:${key}`, route.params[key]);
							}
							if (route.data['breadcrumbs']) {
								route.data['breadcrumbs'] = route.data[
									'breadcrumbs'
								].replace(`:${key}`, route.params[key]);
							}
							if (route.data['description']) {
								route.data['description'] = route.data[
									'description'
								].replace(`:${key}`, route.params[key]);
							}
						}
					...
					if (tmpUrl === '/') {
						paths.push(
							RoutingService.createBreadcrumb(route, tmpUrl)
						);
					} else {
						url += tmpUrl;
