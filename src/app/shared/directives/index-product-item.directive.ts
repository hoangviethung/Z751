...
export class IndexProductItemDirective implements AfterViewInit {
	...
	setEffect() {
		...
		const currentHeight = Number(itemDescription.getAttribute('data-default-height'));
		itemDescription.style.height = `${currentHeight}px`;
	}

	@HostListener('mouseenter') onMouseEnter() {
		const itemDescription = this.elementRef.nativeElement.querySelector('.description');
		itemDescription.style.height = `${Number(itemDescription.getAttribute('data-hovered-height'))}px`;
	}

	@HostListener('mouseleave') onMouseLeave() {
		const itemDescription = this.elementRef.nativeElement.querySelector('.description');
		itemDescription.style.height = `${Number(itemDescription.getAttribute('data-default-height'))}px`;
	}
