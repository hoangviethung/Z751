...
export class ContentComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		this.subscriptions.push(
			this.routingService.onChange.subscribe((value: any) => {
				if (value && value[value.length - 1]) {
					...
					this.description =
						value[value.length - 1].data['description']
				}
		...
		this.subscriptions.push(
			this.layoutStore.sidebarLeftElementHeight.subscribe(
				(value: number) => {
	...
	private setContentMinHeight(): void {
		if (this.navigationEnd) {
			...
			const headerFooterOffsetHeight =
				this.headerService.offsetHeight +
				this.footerService.offsetHeight

			if (this.layout === 'fixed') {
				heightStyle =
					this.windowInnerHeight - this.footerService.offsetHeight
			} else {
