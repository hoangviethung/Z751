...
export class SidebarLeftComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	setSidebarListeners(): void {
		...
		this.subscriptions.push(
			this.layoutStore.isSidebarLeftMouseOver.subscribe(
				(value: boolean) => {
		...
		this.subscriptions.push(
			this.layoutStore.isSidebarLeftExpandOnOver.subscribe(
				(value: boolean) => {
					this.isSidebarLeftExpandOnOver = value
					if (
						this.windowInnerWidth > 767 &&
						this.isSidebarLeftCollapsed !== undefined
					) {
						this.layoutStore.sidebarLeftCollapsed(value)
					}
				}
		...
		this.subscriptions.push(
			this.layoutStore.isSidebarLeftCollapsed.subscribe(
				(value: boolean) => {
					this.isSidebarLeftCollapsed = value
					if (this.windowInnerWidth <= 767) {
						if (value) {
							this.renderer2.removeClass(
								this.wrapperService.wrapperElementRef
									.nativeElement,
								'sidebar-open'
							)
						} else {
	...
	setMenuListeners(url): void {
		if (url === '/') {
			...
			const primaryOutlet = this.router.parseUrl(url).root.children[
				PRIMARY_OUTLET
			]
			if (primaryOutlet) {
	...
	private findItemsByUrl(
		...
		items.forEach((item: Item) => {
			if (url.includes(item.routeActive)) {
	...
	private setMenuTogglesListeners(): void {
		...
		this.ngZone.runOutsideAngular(() => {
			this.sidebarLeftToggleDirectives.forEach(
				(menuToggle: SidebarLeftToggleDirective) => {
					this.toggleListeners.push(
						this.renderer2.listen(
							menuToggle.elementRef.nativeElement,
							'click',
							(event) => {
								...
								if (menuToggle.item.isCollapsed) {
									...
									menuToggle.item.isCollapsed = !menuToggle
										.item.isCollapsed
								}
								this.changeDetectorRef.detectChanges()
							}
	...
	private setSidebarHeight(): void {
		if (this.layout === 'fixed') {
			const height =
				this.windowInnerHeight - this.headerService.offsetHeight
